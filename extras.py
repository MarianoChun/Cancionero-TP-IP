...
def dameLetraApretada(key):
    if key == K_a:
        ...
        return("z")
    elif key == K_9: # Se agrega 9 por la cancion 9 de julio de callejeros
        return("9")
    elif key == K_KP_MINUS:
        return("-")
    elif key == K_SPACE:
...
def dibujarNombreRecords(screen,palabraUsuario,titulo):
        defaultFont= pygame.font.Font( pygame.font.get_default_font(), TAMANNO_LETRA)
        # linea horizontal
        pygame.draw.line(screen, (255,255,255), (0, ALTO-70) , (ANCHO, ALTO-70), 5) 
        # Texto ingresado por el usuario
        screen.blit(defaultFont.render(palabraUsuario, 1, COLOR_TEXTO), (190, 570))
        # Leyenda que solicita ingresar el nombre del jugdor
        screen.blit(defaultFont.render("Ingrese su nombre para guardar su record", 1, COLOR_PELI), (ANCHO//2-len(titulo)*TAMANNO_LETRA//4,(TAMANNO_LETRA)))

def dibujarTexto(texto,fuente,color,screen,x,y): # Con esta funcion escribimos texto en un punto determinado de la pantalla
    textobj = fuente.render(texto,1,color)
    textrect = textobj.get_rect()
    textrect.topleft = (x,y)
    screen.blit(textobj, textrect)

def textoBoton(screen,boton,texto): # Con esta funcion escribimos texto dentro de los rectangulos/botones creados.
    
    fuenteBotones = pygame.font.SysFont("Arial",TAMANNO_LETRA)
    textoBotones = fuenteBotones.render(texto,True,(255,5,5))
    screen.blit(textoBotones,boton)
